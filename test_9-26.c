...
//{
	...
	//char arr[] = { 'a','b','c','d','e','f' };没有结束符在strlen中不知道什么时候遇到结束符
	//printf("%d\n", strlen(arr));//随机值
	//printf("%d\n", strlen(arr + 0));//随机值
...
//int main()
//{
	//char arr[] = { "abcdef" };在后面会自动补上结束符\0，而对于sizeof会计算\0  但是strlen 是计算\0之前的长度
...
//	printf("%s\n", *cpp[-2] + 3);//cpp加-2  值不发生改变ST    意思就是*（*（cpp-2））-3可能是因为直接指向二级指针
...
//	struct S a, * p = &a;
//	a.a = 99;
//	printf("%d\n",   );//能打印的东西不能是*p.a因为.的优先级高于*，可以打印（*P）.a
//	return 0;
//}
...
//int main()
//{
//	struct stu students[3] = { {9801,"zhang",20},{9802,"wang",19},{9803,"li",18} };
//	fun(students + 1);//根据分析得结构是wang
//	return 0;
//
//}
...
//		}
//		//从右边选出奇数
//		while ((left < right)&&(arr[right] % 2 == 0))//害怕全是奇偶数
//		{
//			right--;
//		}
//		if (left < right)
...
//	int len2 = strlen(str2);//区分辨别使str2不是str的子集
//	if (len1 != len2)
//		return 0;
//	//1.在str1字符串中追加一个str1字符串
//	//strcat
//	//strcat(str1, str2);//strcat不能自己追加函数，后面没有\0就容易崩溃
//	//strncat
...
//size_t=unsigned int     strlen函数有头文件打印haha没有头文件打印hehe
//strlen 函数的实现
...
//对strcpy的实现
//#include<assert.h>
//char * my_strcpy(char* dest, const char * scr)//定义scr不能被修改
//{
//	assert(dest != NULL);
//	assert(scr != NULL);
//	char *ret = dest;
...
//	char arr2[] = "bit";
//	//不能’a'这样容易越界
//	//常量字符串储存在字符常量区，其值不能被改易崩溃例如char *arr1是常量字符串
//	my_strcpy(arr1, arr2);
//	printf("%s\n", arr1);
//	return 0;
//}
